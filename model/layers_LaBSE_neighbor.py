...
class MyEmbedder(nn.Module):
    ...
    def contrastive_loss(self, pos_1, pos_2, neg_value):
        ...
        l_pos = l_pos.view(bsz, 1)   #64，1，1
        l_neg = torch.mm(pos_1.view(bsz, -1), neg_value.t())     #64*2048
        logits = torch.cat((l_pos, l_neg), dim=1)
        logits = logits.squeeze().contiguous()   #64，2049
                ...
                if language_id == 1:
                    with torch.no_grad():
                        if self.neg_queue1 == None:
                            ...
                            self.neg_queue1 = torch.cat((self.neg_queue1, token_data.unsqueeze(0)), dim = 0)  #2*64*20*788
#新的+旧的  每次加一
                    ...
                    with torch.no_grad():
                        if self.neg_queue2 == None:
                            self.neg_queue2 = token_data.unsqueeze(0)  #1*64*20*788
                        else:
                ...
                pos_1 = self.model(pos_batch)            #64 20 788-- 64*788

                neg_value_list = []

                with torch.no_grad():
                    self._model.eval()  #没有梯度

                    pos_2 = self._model(pos_batch)
                    neg_shape = neg_queue.shape   #32，64，20，788 队列中32个 第一个正 64为batchsize 64，20，788为token_data
                    neg_queue = neg_queue.reshape(neg_shape[0]*neg_shape[1], neg_shape[2], -1) #2408,20,768
                    neg_value = self._model(neg_queue) #2408,768
